...
(function(){
  ...
  window.showPlay=function(){
    ...
    el.innerHTML=sceneStart('sf-map','World Map of Guitar','Touch a region. Learn how guitar speaks there before choosing any lesson blocks.','Play is a map, not a menu. Click one place and listen for its hand, pulse, scale colour, and story.','images/character-symbols/Encouraging Face Lightbulb.png','play')+'<div class="sf-stage">'+svg+'</div><div id="sf-drawer" class="sf-drawer">Choose a glowing region.</div></div></div>';
  };
  ...
  window.showCreate=function(){
    ...
    el.innerHTML=sceneStart('sf-create','The Cauldron','Create begins with one object: the cauldron. Ingredients are constraints. The song seed appears after you throw something in.','Do not judge the spark too early. Add one ingredient, catch what bubbles up, then shape it.','images/character-symbols/Celebrator with sparks.png','create')+'<div class="sf-stage"><div class="sf-create-stage">'+buttons(left)+'<div class="sf-cauldron"><div class="sf-bubble">'+esc(c.prompt||'Add one ingredient.')+'</div><div class="sf-pot">◠</div><div class="sf-flame">▲ ▲ ▲</div></div>'+buttons(right)+'</div></div><div id="sf-create-work" class="sf-drawer">'+createDrawer(c)+'</div></div></div>';
  };
