...
(function(){
  ...
  window.showHearth=function(){
    ...
    el.innerHTML=sceneStart('sf-brain','The Hearth Brain','One brain. Eight learning systems. Touch a glowing region only when you want detail.','Start with the brain itself. No dashboard. Let the picture tell you where learning lives.','images/character-full/Thinking.png')+'<div class="sf-stage">'+svg+'</div><div id="sf-drawer" class="sf-drawer">Touch one glowing brain region.</div></div></div>';
  };

  // PLAY: only world map first.
  window.showPlay=function(){
    inject(); const el=panel(); if(!el)return; const rs=window.WORLD_MAP_REGIONS||[];
    let svg='<svg viewBox="0 0 900 560"><rect width="900" height="560" fill="#080704"/><g opacity=".06" stroke="'+GOLD+'"><path d="M0 140H900M0 280H900M0 420H900M150 0V560M300 0V560M450 0V560M600 0V560M750 0V560"/></g><g fill="#d4af6909" stroke="#d4af6934"><path d="M80,130 Q60,200 100,260 L160,260 Q210,230 235,190 Q220,140 170,115 Q120,95 80,130Z"/><path d="M250,320 Q238,385 270,500 Q300,520 305,455 Q300,370 278,328Z"/><path d="M420,145 Q485,125 525,170 Q505,235 445,230 Q410,190 420,145Z"/><path d="M455,255 Q520,245 535,330 Q525,420 482,440 Q455,365 455,255Z"/><path d="M570,290 Q640,280 660,340 Q620,385 560,360Z"/><path d="M610,180 Q700,195 735,270 Q670,250 600,198Z"/><ellipse cx="790" cy="220" rx="25" ry="18"/></g>';
    rs.forEach(r=>{svg+='<g onclick="SceneFirst.openPlay(\''+esc(r.id)+'\')"><circle cx="'+r.coords[0]+'" cy="'+r.coords[1]+'" r="18" fill="'+r.color+'" opacity=".12"><animate attributeName="r" values="12;24;12" dur="3.2s" repeatCount="indefinite"/></circle><circle cx="'+r.coords[0]+'" cy="'+r.coords[1]+'" r="6" fill="'+r.color+'" opacity=".65"/><text x="'+r.coords[0]+'" y="'+(r.coords[1]-16)+'" text-anchor="middle" fill="'+r.color+'" font-family="JetBrains Mono" font-size="8">'+esc(r.name)+'</text></g>';}); svg+='</svg>';
    el.innerHTML=sceneStart('sf-map','World Map of Guitar','Touch a region. Learn how guitar speaks there before choosing any lesson blocks.','Play is a map, not a menu. Click one place and listen for its hand, pulse, scale colour, and story.','images/character-symbols/Encouraging Face Lightbulb.png')+'<div class="sf-stage">'+svg+'</div><div id="sf-drawer" class="sf-drawer">Choose a glowing region.</div></div></div>';
  };
  ...
  window.showStudy=function(){
    ...
    el.innerHTML=sceneStart('sf-key','The Key Chamber','Eight concentric rings. Each ring is a level. Move through the doors systematically: define, draw, do, then unlock.','Now the chamber has order: choose a level ring, then open the next lock on that ring. No random study wandering.','images/character-symbols/Thinking Question Mark.png')+'<div class="sf-stage">'+svg+'</div><div class="sf-drawer"><div class="sf-kicker" style="color:'+lvl.color+'">'+lvl.id+' · '+lvl.name+' · '+opened+'/'+activeTopics.length+' opened</div><button class="sf-primary" style="--sf:'+lvl.color+'" onclick="StudyKey.openSession(\''+esc(current?current.c.id:'')+'\',\''+esc(current?current.t.id:'')+'\')">Turn Today’s '+lvl.id+' Key</button> <span style="color:var(--dim);margin-left:8px">'+esc(current?current.t.title:'No lock in this level')+'</span><div class="sf-chiprow">'+STUDY_LEVELS.map((l,i)=>'<button onclick="SceneFirst.studyLevel('+(i+1)+')" style="color:'+l.color+';border-color:'+l.color+'55">'+l.id+'</button>').join('')+'</div></div></div></div>';
  };


  // CREATE: central cauldron first; workstation appears after ingredient.
  window.showCreate=function(){
    inject(); const el=panel(); if(!el)return; const ing=window.CAULDRON_INGREDIENTS||[]; const c=read('hearth-create-current',{prompt:'Add one ingredient.',ingredients:[],notes:'',title:'Untitled Song Seed'}); const left=ing.slice(0,4), right=ing.slice(4);
    const buttons=a=>'<div class="sf-ingredients">'+a.map(x=>'<button style="--c:'+x.color+'" onclick="SceneFirst.addCreate(\''+esc(x.id)+'\')"><span>'+esc(x.symbol)+'</span> '+esc(x.name)+'</button>').join('')+'</div>';
    el.innerHTML=sceneStart('sf-create','The Cauldron','Create begins with one object: the cauldron. Ingredients are constraints. The song seed appears after you throw something in.','Do not judge the spark too early. Add one ingredient, catch what bubbles up, then shape it.','images/character-symbols/Celebrator with sparks.png')+'<div class="sf-stage"><div class="sf-create-stage">'+buttons(left)+'<div class="sf-cauldron"><div class="sf-bubble">'+esc(c.prompt||'Add one ingredient.')+'</div><div class="sf-pot">◠</div><div class="sf-flame">▲ ▲ ▲</div></div>'+buttons(right)+'</div></div><div id="sf-create-work" class="sf-drawer">'+createDrawer(c)+'</div></div></div>';
  };
  function createDrawer(c){
    if(!c.ingredients||!c.ingredients.length)return 'Choose one ingredient. The workstation opens after the cauldron speaks.';
  ...
  window.showMastery=function(){
    ...
    el.innerHTML=sceneStart('sf-phoenix','Mastery · Phoenix Rising','Mastery is not just proof. Proof is the floor. Mastery is going beyond: studying those who crossed the boundary, transforming skill into voice, and rising into a new form.','You do not master by finishing. You master by returning, transforming, and going beyond what the map first showed you.','images/character-symbols/Celebrator with sparks.png')+'<div class="sf-stage">'+phoenix+'</div><div id="sf-drawer" class="sf-drawer">Choose a phoenix seal: hear beyond, find your voice, transform skill, or transmit the fire.</div></div></div>';
  };
