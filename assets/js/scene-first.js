...
(function(){
  ...
  window.showCreate=function(){
    ...
    el.innerHTML=sceneStart('sf-create','The Cauldron','Create begins with one object: the cauldron. Ingredients are constraints. The song seed appears after you throw something in.','Do not judge the spark too early. Add one ingredient, catch what bubbles up, then shape it.','images/character-symbols/Celebrator with sparks.png','create')+'<div class="sf-stage" style="text-align:center"><img src="assets/svg/cauldron.svg" style="width:100%;max-width:280px;height:auto;margin:0 auto;display:block" /><div style="position:relative;margin-top:-100px;z-index:2;max-width:420px;margin-left:auto;margin-right:auto">'+esc(c.prompt||'Select ingredients and stir the cauldron.')+'</div></div><div class="sf-stage"><div class="sf-ingredients" style="display:flex;flex-wrap:wrap;justify-content:center;gap:6px">'+allBtns+'</div>'+stirBtn+hint+'</div><div id="sf-create-work" class="sf-drawer">'+createDrawer(c)+'</div></div></div>';
  };
