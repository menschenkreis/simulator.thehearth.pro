...
| `adapters/dictionary-controller.js` | Renders and filters the legacy glossary/dictionary panel. |
...
| `adapters/doing-shell-viewer.js` | Renders the outer Doing shell and progress header. |
...
| `adapters/doing-panel-controller.js` | Opens and coordinates the Doing panel using the smaller Doing adapters. |
...
| `adapters/knowing-study-dashboard-viewer.js` | Renders the Study Lab dashboard screen. |
...
| `adapters/link-deposit-controller.js` | Handles the legacy YouTube link deposit popup and API save flow. |
...
| `adapters/header-tools-controller.js` | Coordinates legacy search, progress, and settings header panels. |
